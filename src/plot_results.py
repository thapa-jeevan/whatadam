...
for f in args.folders:
    ...
    for s in range(len(args.seeds)):
        for a in args.approaches:
            ...
            ref_random = np.repeat(np.reshape(np.diag(data[f]['random'][s]), (1, data[f]['random'][s].shape[1])),
                                   data[f]['random'][s].shape[0], axis=0)
...
for f in data.keys():
    for a in data[f].keys():
        ...
        # Do the plot
        plt.errorbar(1 + np.arange(acc.shape[1]) + jit, np.mean(acc, axis=0), yerr=ci, fmt='-' + mark, markersize=6,
                     capsize=3)
        # leg.append(f+' : '+a)
        leg.append(a)
        # Print
