...
class Appr(object):
    ...
    def _get_optimizer(self, lr=None, task=None):
        if lr is None: lr = self.lr
        print("Used AdamW optimizer!")

        if task == 0 or task is None:
            ...
            for n, p in self.model.named_parameters():
                if n in self.mask_back:
                    self.update_masks[p] = self.mask_back[n]
            return AdamW(self.model.parameters(), lr=lr, update_masks=self.update_masks)
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid):
        ...
        self.optimizer = self._get_optimizer(lr, t)
        ...
        # Weights mask
        self.mask_back = {}
        mask_count, total = 0, 0
        for n, _ in self.model.named_parameters():
            vals = self.model.get_view_for(n, self.mask_pre)
            if vals is not None:
                self.mask_back[n] = 1 - vals
            try:
                mask_count += (1 - self.mask_back[n]).sum().item()
                total += np.prod(self.mask_back[n].size()).item()
            except:
                continue

        mask_percent = 100 * mask_count / total
        print("-" * 100)
        print('Masking {:.2f}% of weights'.format(mask_percent))
        print("-" * 100)
        return mask_percent
