...
class Appr(object):
    ...
    def __init__(self, model, nepochs=100, sbatch=64, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=1000,
                 generations=20, args=None):
        self.model = model
            ...
            for g in range(self.generations):
                ...
                for p in range(self.P):
                    ...
                    for e in range(self.nepochs):
                        ...
                        clock2 = time.time()
                        print(
                            '| Generation {:3d} | Path {:3d} | Epoch {:3d}, time={:5.1f}ms/{:5.1f}ms | Train: loss={:.3f}, acc={:5.1f}% |'.format(
                                g + 1, p + 1, e + 1, 1000 * self.sbatch * (clock1 - clock0) / xtrain.size(0),
                                1000 * self.sbatch * (clock2 - clock1) / xtrain.size(0), train_loss, 100 * train_acc),
                            end='')
                        # Valid
